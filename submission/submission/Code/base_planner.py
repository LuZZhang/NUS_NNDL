    ...
    def map_callback(self):
        ...
        for i in range(self.world_width):
            for j in range(self.world_height):
                ...
                #occupancy_grid->data[height*occupancy_grid.info.width + width]
        #new_grid=ndimage.grey_dilation(grid, size=(int(self.inflation_ratio),int(self.inflation_ratio)))
        #old_grid[old_grid==-1] = 0
        '''
        dilation=ndimage.grey_dilation(old_grid, size=(int(self.inflation_ratio),int(self.inflation_ratio)))
        for i in range(self.world_width):
            for j in range(self.world_height):
                if(grid[i][j]==-1):
                    continue
